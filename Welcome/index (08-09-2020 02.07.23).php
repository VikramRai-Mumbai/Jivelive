 ...
 <div  class="container">
   ...
   <?php
		  ...
		  {        
			    ...
			    $upateorderid= "UPDATE user SET annual_plan='1-Month-Free-Trial', pur_date='".date('Y-m-d', strtotime($trial_date. ' + 30 days'))."',expiry_date='".$expiry_date."', txn_status='Active'  where annual_plan='NO PLAN' and username='".$_SESSION['username']."' AND status='Active' ";
				 
			   $upateorderidrun = $conn2->query($upateorderid);
			        $_SESSION['annual_plan']= "1-Month-Free-Trial";					
...
?>		   
    ...
    if(isset($_GET['cc']))
	  {
			 ...
			 $cur_time=date('H:i:s',time());
			  $cc="UPDATE `chats` SET closeby='".$_SESSION['username']."' , cdate='".$cur_date."' , ctime='".$cur_time."' , status='Inactive' WHERE cid='".$_GET['cc']."' AND status='Active' ";
					 ...
					 while($rowfb = $cidrun->fetch_array())
					  {
						  if($rowfb['sender'] == $_SESSION['username']){
			                 ...
			                 if ($conn2->query($cc) === TRUE) {
								            ...
								            $ift="INSERT INTO `friends` (`fid`, `cid`, `username`, `friendusername`, `date`, `time`, `status`) VALUES (NULL, '".$_SESSION['cchatid']."', '".$_SESSION['username']."', '".$rowfb['receiver']."', '".$cur_date."', '".$cur_time."', 'Following')";
			                 ...
			                 if ($conn2->query($cc) === TRUE) {
								                     ...
								                     $ift="INSERT INTO `friends` (`fid`, `cid`, `username`, `friendusername`, `date`, `time`, `status`) VALUES (NULL, '".$_SESSION['cchatid']."', '".$rowfb['sender']."', '".$_SESSION['username']."', '".$cur_date."', '".$cur_time."', 'Following')";
			                               if ($conn2->query($ift) === TRUE) {}
 ...
 <?php
           ...
           if(isset($_GET['report']))
	  {
			  ...
			  $cc="UPDATE `chats` SET reportby='".$_SESSION['username']."' ,report='".$report."'  WHERE cid='".$_SESSION['cchatid']."' AND status='Active' ";
			  if ($conn2->query($cc) === TRUE) {
           ...
           if(isset($_SESSION['cchatid']))
	      {
             ...
             if ($sameidrun->num_rows > 0) {}
			 else
			  {
				     ...
				     $qforcid="SELECT cid FROM chats Where sender != '".$_SESSION['username']."' AND sender NOT IN (SELECT friendusername FROM friends where status = 'Friend' AND username='".$_SESSION['username']."' UNION SELECT username FROM friends where status = 'Friend' AND friendusername='".$_SESSION['username']."') AND receiver IS NULL AND status = 'Active' ORDER BY cid LIMIT 1 ";
					 $cidrun = $conn2->query($qforcid);
				 if ($cidrun->num_rows > 0) {
									...
									<div>
											...
											<!-- Example split danger button -->
												<div class="btn-group dropleft">
												  ...
												  <div class="dropdown-menu">
													<a id='frndmodal' class="dropdown-item"  href="#">Friends</a>
													<a id='optionfollowingmodal'class="dropdown-item disabled" href="#">Following</a>
													<a id='optionfollowersmodal'class="dropdown-item disabled" href="#">Followers</a>
										...
										<div class="twPc-divUser">
											<div class="twPc-divName">
												<a style='text-decoration:none;' href="#"><?php /*echo $_SESSION['user_n'];*/echo "  ".$_SESSION['user_dname']." ";if($_SESSION['user_prime'] > 0){ echo " <sup ><svg  style='margin-top:0px;' width='2em' height='2em' viewBox='0 0 20 20' class='bi bi-patch-check-fll' fill='orange' xmlns='http://www.w3.org/2000/svg'>
										...
										<div class="twPc-divStats">
											...
											<?php 
											 ...
											 $flistme="SELECT fid,cid,friendusername,date FROM friends where status != 'Following' AND status != 'Blocked' AND status != 'Reported' AND username='".$_SESSION['username']."' UNION SELECT fid,cid,username,date FROM friends where status != 'Following' AND status != 'Blocked' AND status != 'Reported' AND friendusername='".$_SESSION['username']."' ORDER BY fid DESC ";
											 $myfollistme="SELECT fid,cid,friendusername,date FROM friends where status != 'Blocked' AND status != 'Reported' AND username='".$_SESSION['username']."' UNION SELECT fid,cid,username,date FROM friends where status != 'Blocked' AND status != 'Reported' AND friendusername='".$_SESSION['username']."' ORDER BY fid DESC ";
											 $myfolleristme="SELECT fid,cid,friendusername,date FROM friends where status != 'Blocked' AND status != 'Reported' AND status != 'Friend' AND username='".$_SESSION['username']."' UNION SELECT fid,cid,username,date FROM friends where status != 'Blocked' AND status != 'Reported' AND friendusername='".$_SESSION['username']."' ORDER BY fid DESC ";
											 ...
											 if ($result=mysqli_query($conn4,$myfolleristme)){$_SESSION['myfolleristme']=mysqli_num_rows($result);mysqli_free_result($result);}
											 /*
...
<ul class="nav nav-tabs" id="myTab" role="tablist">
  <li class="nav-item">
    <a class="nav-link <?php if(isset($_GET['timeline'])){ echo ""; }if(isset($_GET['whatsnew'])){echo "";}elseif(isset($_GET['mypost'])){echo "active";}else{echo "";} ?>" id="MyPost-tab" data-toggle="tab" href="#MyPost" role="tab" aria-controls="MyPost" aria-selected="false">My Post</a>
  </li>
  <li class="nav-item">
    <a class="nav-link <?php if(isset($_GET['timeline'])){ echo "active"; }if(isset($_GET['whatsnew'])){echo "";}elseif(isset($_GET['mypost'])){echo " ";}else{echo "";} ?>" id="Timeline-tab" data-toggle="tab" href="#Timeline" role="tab" aria-controls="Timeline" aria-selected="false">Timeline</a>
  </li>
  <li class="nav-item">
...
<div class="tab-content" id="myTabContent">
  ...
  <a href='/Welcome/?publishnewpost=1'><button style ='margin-top:2px' class='btn btn-outline-success btn-sm float-right' type='button'>Publish New Post</button></a>
  <br><br>
  <div id='fetchpost'>
...
if(isset($_SESSION['username']))
	      {
			  ...
			  if (isset($_GET['mypost'])) {
                ...
                $offset = ($mypost_page-1) * $no_of_records_per_page; 
				 $count="SELECT pid FROM post WHERE username = '".$_SESSION['username']."'  AND status='Active' ORDER BY pid DESC ";		  
		         $countrun = $conn3->query($count);
				 $all=0;
				if ($countrun->num_rows > 0) { while($cntrow = $countrun->fetch_array()) {$all=$all+1;} } else{$all=0;}
				$total_pages_mypost = ceil($all / $no_of_records_per_page);
			  //////////  pagination    ////////////////
			  
			 $mypost="SELECT * FROM post WHERE username = '".$_SESSION['username']."'  AND status='Active' ORDER BY pid DESC LIMIT $offset , $no_of_records_per_page ";		  
							               ...
							               <div class='col-xs-10 col-md-11'>
												<div id='userinfo'>
													<a href='#'>
														...
														<b>".$mypostrow['user_dname']." ";if($_SESSION['user_prime'] > 0){ echo " <sup ><svg  width='2em' height='2em' viewBox='0 0 20 20' class='bi bi-patch-check-fll' fill='orange' xmlns='http://www.w3.org/2000/svg'>
                                                           <path fill-rule='evenodd' d='M10.067.87a2.89 2.89 0 0 0-4.134 0l-.622.638-.89-.011a2.89 2.89 0 0 0-2.924 2.924l.01.89-.636.622a2.89 2.89 0 0 0 0 4.134l.637.622-.011.89a2.89 2.89 0 0 0 2.924 2.924l.89-.01.622.636a2.89 2.89 0 0 0 4.134 0l.622-.637.89.011a2.89 2.89 0 0 0 2.924-2.924l-.01-.89.636-.622a2.89 2.89 0 0 0 0-4.134l-.637-.622.011-.89a2.89 2.89 0 0 0-2.924-2.924l-.89.01-.622-.636zm.287 5.984a.5.5 0 0 0-.708-.708L7 8.793 5.854 7.646a.5.5 0 1 0-.708.708l1.5 1.5a.5.5 0 0 0 .708 0l3-3z'/>
                                              ...
                                              <div class='comment-text'>
											   <h6 id='heading' class='font-weight-bold'> ".$mypostrow['post_head']."</h6>
											
												  <p style='font-size: 14px;'><b>".$mypostrow['post_subhead']."</b></p>
												  
													<p>".$mypostrow['post_body']."

												";
												if(!empty($mypostrow['ref_link'])){ echo "    <a href='".$mypostrow['ref_link']."'> <b>Click for ref</b></a></p>"; }else{echo "</p>";}	
												if(!empty($mypostrow['post_embed'])){ echo " <div style='' class='embedimg img-thumbnail mx-auto embed-responsive embed-responsive-16by9 float-left '><iframe class='embed-responsive-item' src='".$mypostrow['post_embed']."'></iframe></div>"; }	
												if(!empty($mypostrow['post_img'])){ echo "<img src='".$mypostrow['post_img']."' class='rounded mx-auto d-block img-thumbnail float-left img-responsive' alt=''  />"; }	
												if(!empty($mypostrow['post_img2'])){ echo "<img src='".$mypostrow['post_img2']."' class='rounded mx-auto d-block img-thumbnail float-left img-responsive' alt=''  />"; }	
												echo "
											</div>
...
if(isset($_SESSION['username']))
	      {
            ...
            <div class='panel-heading'>
                ...
                <span class='label label-info w3-text-green'> <b>Page : ".$timeline_page."  /  ".$total_pages_timeline." </b></span>
            </div>
            <div class='panel-body'>
                <ul class='list-group'>
                   ";
									  ...
									  echo "
									   <li class='list-group-item'>
							               ...
							               <div class='col-xs-10 col-md-11'>
												<div id='userinfo'>
													<a style='text-decoration:none;font-weight:bold' href='#'>
														<b>".$mytimeline['user_dname']." ";if($mytimeline_ud['prime_user'] > 0){ echo " <sup ><svg  width='2em' height='2em' viewBox='0 0 20 20' class='bi bi-patch-check-fll' fill='orange' xmlns='http://www.w3.org/2000/svg'>
  <path fill-rule='evenodd' d='M10.067.87a2.89 2.89 0 0 0-4.134 0l-.622.638-.89-.011a2.89 2.89 0 0 0-2.924 2.924l.01.89-.636.622a2.89 2.89 0 0 0 0 4.134l.637.622-.011.89a2.89 2.89 0 0 0 2.924 2.924l.89-.01.622.636a2.89 2.89 0 0 0 4.134 0l.622-.637.89.011a2.89 2.89 0 0 0 2.924-2.924l-.01-.89.636-.622a2.89 2.89 0 0 0 0-4.134l-.637-.622.011-.89a2.89 2.89 0 0 0-2.924-2.924l-.89.01-.622-.636zm.287 5.984a.5.5 0 0 0-.708-.708L7 8.793 5.854 7.646a.5.5 0 1 0-.708.708l1.5 1.5a.5.5 0 0 0 .708 0l3-3z'/>
														</svg></sup>";}else{} echo"  </b></a>
													<div class='mic-info'>
		...
		<p class='card-text text-secondary'>You can do payment of plan using highly secured Razorpay Gateway</p>
		   ...
		   <table class='w3-small'>
						  ...
						  <tr>
							...
							<td class='w3-text-green'><b>Unlimited</b></td>
						  </tr>
						  <tr>
							<td>Unknown Chatting</td>
		   ...
		   <table class='w3-small'>
			  <tr>
			   ...
			   <tr>
				<td>Type of Plan</td>
				<td class='w3-text-green' style='font-size:14px'><b><i style='' class='fa'>&#xf156;</i> 1 <s class='text-danger'> 25</s></b></td>
	            <td class='w3-text-green'><b><i style='' class='fa'>&#xf156;</i> 5 <s class='text-danger'> 100</s></b></td>
...
<?php } else { header("Location: http://jivelive.jamamo.in/security/?pid=welcome&error=jfajhjchuy87859784cn7t8y0w758y87ytc8w7t9w5jtyc98wmut894375873ck7u8937587ct"); } ?>