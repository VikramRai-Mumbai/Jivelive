...
if(is_array($_FILES)) {
						       ...
						       $S_Profile1= "http://jivelive.jamamo.in/JLIMG/user_post/".$_SESSION['user_handle']."/".$file_name;
								...
								$update1 = "UPDATE post SET post_img2='".$S_Profile1."' WHERE pid='".$_SESSION['postlastid']."'";

								if ($conn3->query($update1) === TRUE) {
									
									$date= date('Y-m-d',time());
					                $time= date('H:i:s',time());
									$post_log = "Insert into post_log(username,name,pid,remarks,date,time) VALUES('".$_SESSION['username']."','".$_SESSION['user_dname']."','".$_SESSION['postlastid']."','Post Media 2 Uploaded','".$date."','".$time."') ";
